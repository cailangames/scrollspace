...
def get_background_data_and_map(im, name, gb_code, offset=37, debug=False):
    ...
    tilemap_array += f"const uint8_t {filename}_ntiles = {len(unique_tiles)};\n"
    tilemap_array += f"const uint8_t {filename}_map[{stride*int(ntiles/stride)}];\n"
    tilemap_array += f"const uint8_t {filename}_map[] = "
    tilemap_array += "{"
    for i in range(ntiles):
        if i % 20 == 0:
           tilemap_array += "\n  "
...
gb_code_sprite_rgb = {356:0, 679:1, 434:2, 64:3}  # GBStudio sprite pallete
gb_code_bkg_rgb = {679:0, 434:1, 202:2, 64:3}  # GBStudio bkg pallete
gb_code_sprite_indexed = {0:0, 1:1, 2:2, 4:3}  # GBStudio sprite pallete
gb_code_bkg_indexed = {1:0, 2:1, 3:2, 4:3}  # GBStudio bkg pallete
